...
ProtoObject* proto_compose_action_perflow_message(actionState *action_state){
	ActionState action_perflow = ACTION_STATE__INIT;
        ...
        long start_time = action_state->start_time;
        action_perflow.start_time = start_time;
	long last_pkt_time =action_state->last_pkt_time;
	action_perflow.last_pkt_time = last_pkt_time;
...
int local_action_get_one_perflow(int hash, int cxid){
	...
	while(action_state != NULL){
		...
		ActionState action_perflow;
		action_state__init(&aaction_perflow);
